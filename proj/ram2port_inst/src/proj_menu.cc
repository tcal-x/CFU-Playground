...
void do_custom(void) {
  for (int a = 0; a < 6; a++) {
      int loca[6];
      printf("---- check free-running writes:\n");
      for (int b = 0; b < 6; b++) {
          loca[b] = cfu_op0(0, b, b);
      }
      for (int b = 0; b < 6; b++) {
          printf("%x ", loca[b]);
      }
  ...
  const int base = 1024;
  printf("---- check write/read, base %x\n", base);
  for (int b = 0; b < 6; b++) {
      cfu_op1(0, base+b, base+b);
  }
  for (int b = 0; b < 6; b++) {
      printf("%lx ", cfu_op0(0, base+b, base+b));
  }
  printf("\n");
